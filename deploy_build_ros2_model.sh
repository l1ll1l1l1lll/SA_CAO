...
# (ros2nodeinterface.cpp, main.cpp or <modelname>_ctrlr_host.cpp)
...
if [ -f "$MODEL_REF_LIST" ] ; then
    ...
    do
        ...
        if [ -f "$mdlRefArchive" ] ; then
            ...
            # Remove model reference tgz
            rm -f "$mdlRefArchive"
        fi
    done < "$MODEL_REF_LIST"
fi
# Remove model reference text file
rm -f "$MODEL_REF_LIST"

# Extract custom message package archives (if needed)
CUSTOM_MSG_LIST="$ARCHIVE_DIR/$ARCHIVE_BASE"CustomMsgs.txt
if [ -f "$CUSTOM_MSG_LIST" ] ; then
    customMsgPkgs=()
    while IFS= read -r customMsgArchive
    do
        # Trim whitespaces and newlines to account for OS-specific text
        customMsgArchive=$(trim "$ARCHIVE_DIR/$customMsgArchive")

        echo "Extracting custom message archive $customMsgArchive"

        # Extract archive if it exists
        if [ -f "$customMsgArchive" ] ; then
            # Create folder
            CUSTOMMSG_DIR="$PROJECT_DIR/$(toLowerCase $(basename "$customMsgArchive" .tgz))"
            rm -fr "$CUSTOMMSG_DIR"
            # Extract archive into created folder
            tar -C "$PROJECT_DIR" -xf "$customMsgArchive"
            # Remove custom message tgz
            rm -f "$customMsgArchive"
        fi
    done < "$CUSTOM_MSG_LIST"
fi
# Remove custom message text file
rm -f "$CUSTOM_MSG_LIST"
