...
// Function for MATLAB Function: '<S12>/Force to stop'
boolean_T Sub_diablo::Sub_diablo_isequal(const int32_T varargin_1[2], const
  real_T varargin_2[2])
{
  int32_T k;
  boolean_T b_p;
...
void Sub_diablo::Sub_diab_Subscriber_setupImpl_o(const
  ros_slros2_internal_block_Sub_T *obj)
{
  ...
  for (int32_T i = 0; i < 16; i++) {
    // Start for MATLABSystem: '<S5>/SourceBlock'
    Sub_diablo_B.b_zeroDelimTopic[i] = b_zeroDelimTopic[i];
  }

  Sub_Sub_diablo_1483.createSubscriber(&Sub_diablo_B.b_zeroDelimTopic[0],
    qos_profile);
}

void Sub_diablo::Sub_diablo_Subscriber_setupImpl(const
  ros_slros2_internal_block_Sub_T *obj)
{
  rmw_qos_profile_t qos_profile;
  sJ4ih70VmKcvCeguWN0mNVF lifespan;
  ...
  Sub_diablo_B.deadline_k.sec = 0.0;
  Sub_diablo_B.deadline_k.nsec = 0.0;
  lifespan.sec = 0.0;
  lifespan.nsec = 0.0;
  liveliness_lease_duration.sec = 0.0;
  liveliness_lease_duration.nsec = 0.0;
  SET_QOS_VALUES(qos_profile, RMW_QOS_POLICY_HISTORY_KEEP_LAST, (size_t)1.0,
                 RMW_QOS_POLICY_DURABILITY_VOLATILE,
                 RMW_QOS_POLICY_RELIABILITY_RELIABLE, Sub_diablo_B.deadline_k,
  ...
  Sub_Sub_diablo_1432.createSubscriber(&b_zeroDelimTopic[0], qos_profile);
}

void Sub_diablo::Sub_diablo_Publisher_setupImpl(const
  ros_slros2_internal_block_Pub_T *obj)
{
  ...
  static const char_T b_zeroDelimTopic_0[8] = "/vw_vel";
  qos_profile = rmw_qos_profile_default;

  // Start for MATLABSystem: '<S7>/SinkBlock'
  Sub_diablo_B.deadline.sec = 0.0;
  Sub_diablo_B.deadline.nsec = 0.0;
  lifespan.sec = 0.0;
  lifespan.nsec = 0.0;
  liveliness_lease_duration.sec = 0.0;
  liveliness_lease_duration.nsec = 0.0;
  SET_QOS_VALUES(qos_profile, RMW_QOS_POLICY_HISTORY_KEEP_LAST, (size_t)1.0,
                 RMW_QOS_POLICY_DURABILITY_VOLATILE,
                 RMW_QOS_POLICY_RELIABILITY_RELIABLE, Sub_diablo_B.deadline,
                 lifespan, RMW_QOS_POLICY_LIVELINESS_AUTOMATIC,
                 liveliness_lease_duration, (bool)
                 obj->QOSAvoidROSNamespaceConventions);
  ...
  Pub_Sub_diablo_1880.createPublisher(&b_zeroDelimTopic[0], qos_profile);
}

// Model step function
void Sub_diablo::step()
{
  real_T F_i;
  real_T rtb_angels_0;
  real_T rtb_vw_idx_0;
  ...
  b_varargout_1 = Sub_Sub_diablo_1483.getLatestMessage
    (&Sub_diablo_B.rtb_SourceBlock_o2_b_c);
  ...
  if (b_varargout_1) {
    ...
    Sub_diablo_B.In1_a = Sub_diablo_B.rtb_SourceBlock_o2_b_c;
  }
  ...
  //   MATLAB Function: '<S6>/vw_calculator'

  if ((Sub_diablo_B.In1_a.buttons[4] == 1) && (Sub_diablo_DW.prevButtons[4] == 0))
  {
    Sub_diablo_DW.behaviorSwitch--;
  } else if ((Sub_diablo_B.In1_a.buttons[5] == 1) && (Sub_diablo_DW.prevButtons
              [5] == 0)) {
    Sub_diablo_DW.behaviorSwitch++;
  }

  if ((Sub_diablo_DW.behaviorSwitch >= 2.0) || rtIsNaN
      (Sub_diablo_DW.behaviorSwitch)) {
    Sub_diablo_DW.behaviorSwitch = 2.0;
  }

  if (Sub_diablo_DW.behaviorSwitch <= 0.0) {
    Sub_diablo_DW.behaviorSwitch = 0.0;
  }

  if ((Sub_diablo_B.In1_a.axes[7] > 0.9) && (Sub_diablo_DW.prevAxes[7] < 0.1) &&
      (Sub_diablo_DW.prevAxes[7] > -0.1)) {
    Sub_diablo_DW.WanderVelocity += 0.1;
  } else if ((Sub_diablo_B.In1_a.axes[7] < -0.9) && (Sub_diablo_DW.prevAxes[7] <
              0.1) && (Sub_diablo_DW.prevAxes[7] > -0.1)) {
    Sub_diablo_DW.WanderVelocity -= 0.1;
  }

  if ((Sub_diablo_DW.WanderVelocity >= 1.0) || rtIsNaN
      (Sub_diablo_DW.WanderVelocity)) {
    Sub_diablo_DW.WanderVelocity = 1.0;
  }

  if (Sub_diablo_DW.WanderVelocity <= 0.0) {
    Sub_diablo_DW.WanderVelocity = 0.0;
  }

  switch (static_cast<int32_T>(Sub_diablo_DW.behaviorSwitch)) {
   case 0:
    StateWander = 0;
    StateControl = 0;
    break;
  ...
  memcpy(&Sub_diablo_DW.prevButtons[0], &Sub_diablo_B.In1_a.buttons[0], sizeof
         (int32_T) << 7U);
  memcpy(&Sub_diablo_DW.prevAxes[0], &Sub_diablo_B.In1_a.axes[0], sizeof
         (real32_T) << 7U);

  // End of MATLAB Function: '<S2>/MATLAB Function'

  // Switch: '<S2>/Subsume3' incorporates:
  //   Constant: '<S2>/Constant'
  //   DataStoreRead: '<S2>/Data Store Read'

  if (StateControl > 0) {
    // Switch: '<S2>/Subsume4' incorporates:
    ...
    if (StateWander > 0) {
      ...
      rtb_vw_idx_0 = Sub_diablo_DW.WanderVelocity;
      rtb_vw_idx_1 = Sub_diablo_DW.WanderVelocity;
    } else {
      _mm_storeu_pd(&Sub_diablo_B.dv[0], _mm_mul_pd(_mm_set_pd
        (static_cast<real_T>(Sub_diablo_B.In1_a.axes[0]), static_cast<real_T>
         (Sub_diablo_B.In1_a.axes[1])), _mm_set_pd(2.0, 0.7)));
      rtb_vw_idx_0 = Sub_diablo_B.dv[0];
      rtb_vw_idx_1 = Sub_diablo_B.dv[1];
    }

    // End of Switch: '<S2>/Subsume4'
  } else {
    rtb_vw_idx_0 = 0.0;
    rtb_vw_idx_1 = 0.0;
  }

  // End of Switch: '<S2>/Subsume3'

  // MATLABSystem: '<S1>/SourceBlock'
  b_varargout_1 = Sub_Sub_diablo_1432.getLatestMessage
    (&Sub_diablo_B.rtb_SourceBlock_o2_m);
  ...
  if (b_varargout_1) {
    ...
    Sub_diablo_B.In1 = Sub_diablo_B.rtb_SourceBlock_o2_m;
  }
  ...
  for (StateWander = 0; StateWander < 720; StateWander++) {
    ...
    F_i = Sub_diablo_B.In1.ranges[StateWander];

    // MATLAB Function: '<S2>/lidar offset setting' incorporates:
    //   MATLAB Function: '<S20>/MATLAB Function'

    rtb_angels_0 = tmp[StateWander];

    // MATLAB Function: '<S9>/MATLAB Function' incorporates:
    //   DataStoreRead: '<S9>/Data Store Read'
    //   DataStoreRead: '<S9>/Data Store Read1'

    if (F_i > Sub_diablo_DW.max_scan) {
      F_i = (rtNaN);
    }

    Sub_diablo_B.x[StateWander] = 0.0;
    Sub_diablo_B.b_x[StateWander] = 0.0;
    if (F_i > 0.0) {
      F_i = exp(-Sub_diablo_DW.lambda * F_i) * 10.0;
      Sub_diablo_B.x[StateWander] = F_i * cos(rtb_angels_0);
      Sub_diablo_B.b_x[StateWander] = F_i * sin(rtb_angels_0);
    }
  }

  // MATLAB Function: '<S9>/MATLAB Function'
  rtb_angels_0 = Sub_diablo_B.x[0];
  F_i = Sub_diablo_B.b_x[0];
  for (StateWander = 0; StateWander < 719; StateWander++) {
    _mm_storeu_pd(&Sub_diablo_B.dv[0], _mm_add_pd(_mm_set_pd
      (Sub_diablo_B.b_x[StateWander + 1], Sub_diablo_B.x[StateWander + 1]),
      _mm_set_pd(F_i, rtb_angels_0)));
    rtb_angels_0 = Sub_diablo_B.dv[0];
    F_i = Sub_diablo_B.dv[1];
  }

  // MATLAB Function: '<S16>/keyboard_vw' incorporates:
  ...
  Sub_diablo_DW.v = (rtb_angels_0 * 0.01 * Sub_diablo_DW.k_v * 0.05 +
                     rtb_vw_idx_0) * 0.19999999999999996 + 0.8 * Sub_diablo_DW.v;
  if (!(Sub_diablo_DW.v <= 0.5)) {
    Sub_diablo_DW.v = 0.5;
  }
  ...
  Sub_diablo_DW.w = (F_i * 0.01 * Sub_diablo_DW.k_w * 0.05 + rtb_vw_idx_1) *
    0.19999999999999996 + 0.8 * Sub_diablo_DW.w;
  if (!(Sub_diablo_DW.w <= 1.0)) {
    Sub_diablo_DW.w = 1.0;
  }
  ...
  //   MATLAB Function: '<S11>/speed limiter'
  //   MATLAB Function: '<S12>/Force to stop'
  //   MATLAB Function: '<S16>/keyboard_vw'

  Sub_diablo_B.BusAssignment.linear.x = Sub_diablo_DW.v * 0.8;
  Sub_diablo_B.BusAssignment.angular.z = Sub_diablo_DW.w * 0.8;

  // MATLAB Function: '<S12>/Force to stop' incorporates:
  //   MATLAB Function: '<S6>/vw_calculator'

  xo[0] = Sub_diablo_B.In1_a.buttons[0];
  xo[1] = Sub_diablo_B.In1_a.buttons[1];
  Sub_diablo_B.dv[0] = 1.0;
  Sub_diablo_B.dv[1] = 0.0;
  if (Sub_diablo_isequal(xo, Sub_diablo_B.dv)) {
    Sub_diablo_DW.stop = 0.0;
  } else {
    Sub_diablo_B.dv[0] = 0.0;
    Sub_diablo_B.dv[1] = 1.0;
    if (Sub_diablo_isequal(xo, Sub_diablo_B.dv)) {
      Sub_diablo_DW.stop = 1.0;
    }
  }

  if (Sub_diablo_DW.stop != 0.0) {
    // BusAssignment: '<S4>/Bus Assignment'
    Sub_diablo_B.BusAssignment.linear.x = 0.0;
    Sub_diablo_B.BusAssignment.angular.z = 0.0;
  }

  // DataStoreWrite: '<S2>/Data Store Write' incorporates:
  //   Constant: '<S2>/k_v'

  Sub_diablo_DW.k_v = 11.288224299065419;

  // DataStoreWrite: '<S2>/Data Store Write1' incorporates:
  //   Constant: '<S2>/k_w'

  Sub_diablo_DW.k_w = 13.43147431035171;

  // DataStoreWrite: '<S2>/Data Store Write2' incorporates:
  //   Constant: '<S2>/lambda'

  Sub_diablo_DW.lambda = 3.2298654636911648;

  // DataStoreWrite: '<S2>/Data Store Write3' incorporates:
  //   Constant: '<S2>/max_scan'

  Sub_diablo_DW.max_scan = 0.47431775700934581;

  // BusAssignment: '<S4>/Bus Assignment' incorporates:
  //   MATLAB Function: '<S4>/[v,w]==>geometry_msgs//Twist'

  Sub_diablo_B.BusAssignment.linear.y = 0.0;
  Sub_diablo_B.BusAssignment.linear.z = 0.0;
  Sub_diablo_B.BusAssignment.angular.x = 0.0;
  Sub_diablo_B.BusAssignment.angular.y = 0.0;

  // MATLABSystem: '<S7>/SinkBlock'
  Pub_Sub_diablo_1880.publish(&Sub_diablo_B.BusAssignment);
}

// Model initialize function
void Sub_diablo::initialize()
{
  // Registration code
  ...
  // Start for MATLABSystem: '<S5>/SourceBlock'
  Sub_diablo_DW.obj.QOSAvoidROSNamespaceConventions = false;
  Sub_diablo_DW.obj.matlabCodegenIsDeleted = false;
  Sub_diablo_DW.obj.isSetupComplete = false;
  Sub_diablo_DW.obj.isInitialized = 1;
  Sub_diab_Subscriber_setupImpl_o(&Sub_diablo_DW.obj);
  Sub_diablo_DW.obj.isSetupComplete = true;

  // Start for MATLABSystem: '<S1>/SourceBlock'
  Sub_diablo_DW.obj_g.QOSAvoidROSNamespaceConventions = false;
  Sub_diablo_DW.obj_g.matlabCodegenIsDeleted = false;
  Sub_diablo_DW.obj_g.isSetupComplete = false;
  Sub_diablo_DW.obj_g.isInitialized = 1;
  Sub_diablo_Subscriber_setupImpl(&Sub_diablo_DW.obj_g);
  Sub_diablo_DW.obj_g.isSetupComplete = true;

  // Start for MATLABSystem: '<S7>/SinkBlock'
  Sub_diablo_DW.obj_k.QOSAvoidROSNamespaceConventions = false;
  Sub_diablo_DW.obj_k.matlabCodegenIsDeleted = false;
  Sub_diablo_DW.obj_k.isSetupComplete = false;
  Sub_diablo_DW.obj_k.isInitialized = 1;
  Sub_diablo_Publisher_setupImpl(&Sub_diablo_DW.obj_k);
  Sub_diablo_DW.obj_k.isSetupComplete = true;
}

// Model terminate function
void Sub_diablo::terminate()
{
  // Terminate for MATLABSystem: '<S5>/SourceBlock'
  if (!Sub_diablo_DW.obj.matlabCodegenIsDeleted) {
    Sub_diablo_DW.obj.matlabCodegenIsDeleted = true;
    if ((Sub_diablo_DW.obj.isInitialized == 1) &&
        Sub_diablo_DW.obj.isSetupComplete) {
      Sub_Sub_diablo_1483.resetSubscriberPtr();//();
    }
  }
  ...
  if (!Sub_diablo_DW.obj_g.matlabCodegenIsDeleted) {
    Sub_diablo_DW.obj_g.matlabCodegenIsDeleted = true;
    if ((Sub_diablo_DW.obj_g.isInitialized == 1) &&
        Sub_diablo_DW.obj_g.isSetupComplete) {
      Sub_Sub_diablo_1432.resetSubscriberPtr();//();
    }
  }
  ...
  if (!Sub_diablo_DW.obj_k.matlabCodegenIsDeleted) {
    Sub_diablo_DW.obj_k.matlabCodegenIsDeleted = true;
    if ((Sub_diablo_DW.obj_k.isInitialized == 1) &&
        Sub_diablo_DW.obj_k.isSetupComplete) {
      Pub_Sub_diablo_1880.resetPublisherPtr();//();
    }
  }
...
RT_MODEL_Sub_diablo_T * Sub_diablo::getRTM()
{
  return (&Sub_diablo_M);
}

const char_T* RT_MODEL_Sub_diablo_T::getErrorStatus() const
{
  return (errorStatus);
}

void RT_MODEL_Sub_diablo_T::setErrorStatus(const char_T* const volatile
  aErrorStatus)
{
