...
// Conversions between SL_Bus_sensor_msgs_Range and sensor_msgs::msg::Range

void convertFromBus(sensor_msgs::msg::Range& msgPtr, SL_Bus_sensor_msgs_Range const* busPtr)
{
  const std::string rosMessageType("sensor_msgs/Range");

  msgPtr.field_of_view =  busPtr->field_of_view;
  convertFromBus(msgPtr.header, &busPtr->header);
  msgPtr.max_range =  busPtr->max_range;
  msgPtr.min_range =  busPtr->min_range;
  msgPtr.radiation_type =  busPtr->radiation_type;
  msgPtr.range =  busPtr->range;
}

void convertToBus(SL_Bus_sensor_msgs_Range* busPtr, const sensor_msgs::msg::Range& msgPtr)
{
  const std::string rosMessageType("sensor_msgs/Range");

  busPtr->field_of_view =  msgPtr.field_of_view;
  convertToBus(&busPtr->header, msgPtr.header);
  busPtr->max_range =  msgPtr.max_range;
  busPtr->min_range =  msgPtr.min_range;
  busPtr->radiation_type =  msgPtr.radiation_type;
  busPtr->range =  msgPtr.range;
}
